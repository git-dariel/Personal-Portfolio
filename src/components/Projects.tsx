...
export default function Projects() {
  return (
    ...
    >
      ...
      <div className="flex flex-col gap-10 rounded-[56px] h-max sm:w-[90%] mx-auto">
        {projectsData.map(({ id, name, description, github, link, image, techStack }) => {
          return (
            <article
              key={id}
              ...
              <div className="rounded-2xl border border-slate-950 overflow-hidden aspect-video min-w-[50%] flex items-center justify-center">
                {image ? (
                  ...
                  <div className="flex items-center justify-center">
                    <p className="font-semibold text-center">I&apos;m working on it...</p>
                  </div>
                )}
              </div>
              ...
              <section className="flex flex-col gap-5 justify-between min-w-[45%] min-h-[200px]">
                <div>
                  <div className="flex items-center justify-between gap-2">
                    <h3 className="font-medium text-xl text-left">{name}</h3>
                    ...
                    {github ? (
                      ...
                      <p className="text-sm text-gray-500">GitHub repository not available</p>
                    )}
                  </div>

                  <div className="flex items-center flex-wrap gap-2 py-3 cursor-pointer">
                    {techStack.map((tech, index) => (
                      <Badge key={index} variant={"default"} className="rounded-full text-sm">
                        {tech}
                      </Badge>
                    ))}
                  </div>
